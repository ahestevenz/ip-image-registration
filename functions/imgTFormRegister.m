function [movingReg, optimizer, metric, tform] = imgTFormRegister(moving, fixed, growthfactor, epsilon, initialradius, iterations, samples, histogrambins, pixels, type)
...
tform = imregtform(moving,fixed,type,optimizer,metric); % get an initial transformation estimate based on a 'type' model: For example, the rigid model is composed by translation and rotation operations.
